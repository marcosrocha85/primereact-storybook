...
try {
  if (mode === 'init') {
    save({ root, branch: git('branch', '--show-current'), base: git('rev-parse', 'HEAD'), phase: 'implementation', attempt: 0, fingerprint: fingerprint() });
  } else if (mode === 'resume') {
    ...
    for (let round = 0; round < limit; round++) {
      ...
      const log = fs.openSync(path.join(attempt, 'codex.log'), 'w');
      console.log(`Codex attempt ${state.attempt}: ${attempt}`);
      const resultFile = path.join(attempt, 'result.json');
      const execution = spawnSync('codex', [...parameters, '--output-schema', path.join(support, 'result.schema.json'), '--output-last-message', resultFile, '-'], {
      ...
      // CLI/auth/transport failures are not presumed to be implementation errors.
      assert.equal(execution.status, 0, `Codex CLI failed (${execution.status ?? execution.error?.message}); inspect ${attempt}/codex.log and resume after fixing the environment`);
      let failure;
      let report;
      try {
        ...
        const validation = spawnSync(process.execPath, [path.join(support, 'validate.mjs'), path.join(directory, 'files.list'), attempt], { cwd: root, stdio: 'inherit' });
        invariant(state);
