...
if(program==='codex'){
 ...
 const result={status:'ready',pr_title:'Review component '+issue,pr_body:'Scoped implementation and validation.',changed_files:['component-'+issue+'.txt'],validations:['npm run build','npm run build-storybook','git diff --check'].map(command=>({command,status:'passed'})),api_review_complete:true,blockers:[]};
 if(state.scenario==='blocked') {result.status='blocked';result.blockers=['Dependency unresolved'];}
...
`;
  for (const command of ['codex', 'gh']) fs.writeFileSync(path.join(bin, command), mock, { mode: 0o755 });
  const run = (...args) => spawnSync('bash', ['scripts/implement-issues.sh', ...args], {
    cwd: repo, encoding: 'utf8', timeout: 30000,
    env: { ...gitEnv, PATH: `${bin}:${process.env.PATH}`, RUNNER_TEST_STATE: state, CHECK_TIMEOUT_SECONDS: '1' }
  });
  return { repo, git, run, state: () => JSON.parse(fs.readFileSync(state, 'utf8')), cleanup: () => fs.rmSync(temp, { recursive: true, force: true }) };
}
