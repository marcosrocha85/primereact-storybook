...
The terminal uses `cli-progress` to show a numbered header such as `1/3 [#24]` and keep one compact status line visible while Codex runs, with a batch progress bar, elapsed time and the current phase. Raw Codex/build and routine Git/GitHub output remains in the run logs instead of flooding the console.

Codex must generate an English Conventional Commit title (`type(scope): description` or `type: description`). The runner validates that title and uses it verbatim for both the commit and pull request.
